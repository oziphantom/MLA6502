        ...
        if not self.src_immediate:
            if self.src_original.isnumeric():
                if int(self.src_original) < 256:
                    raise ValueError("non immediate assing with decimal - are you really sure? in line " + line)
...
def get_line_class(line):
    ...
    if is_line_index_prehibit(line):
        return MLALineClass.index_prehibitor
...
def convert_lines_to_asm(lines, segmentDM, tdm):
    ...
    for line in lines:
        ...
        if clas == MLALineClass.straight_assign:
            ...
            if len(straights) > 0:
                out_string += convert_straight_assingment_block_to_asm(straights, segmentDM, tdm, prehib)
                straights = []
        ...
        elif clas == MLALineClass.index_prehibitor:
...
def convert_straight_assingment_block_to_asm(lines, segmentDM, tdm, prehib):
